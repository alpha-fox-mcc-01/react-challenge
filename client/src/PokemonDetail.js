...
export default function PokemonDetail(props) {
    let loseBackground = 'https://www.elsetge.cat/imagepost/b/51/511378_red-gradient-wallpaper.jpg'
    let winBackground = 'https://www.xmple.com/wallpaper/linear-green-white-gradient-1920x1080-c2-90ee90-fffaf0-a-30-f-14.svg'

    let { name } = useParams()
    const { pokemon, comparePokemon, fetchPokemonDetail } = useFetcher();

    let imgUrlMain = 'https://www.xmple.com/wallpaper/cyan-gradient-white-linear-1920x1080-c2-fffafa-6cd9fc-a-255-f-14.svg'

    let imgUrlCompare = 'https://www.xmple.com/wallpaper/cyan-gradient-white-linear-1920x1080-c2-fffafa-6cd9fc-a-255-f-14.svg'

    let mainPower = 0, comparePower = 0
            ...
            <div className="card" style={cardStyleDetail}>
                <div className="card-body pokemon-detail">
                    <div className='row'>
                        <div className='col-md-3 basic-info'>
                            ...
                            <h5>Notable abilities: </h5>
                                    {pokemon.abilities.map(({ ability }) => <h5 key={ Math.random() }>&bull;&nbsp;{ability.name}</h5>)}
                            {/* {pokemon.stats.map((field, i) => <p className="card-text" key={i} >&bull;&nbsp;{field.stat.name}: {field.base_stat}</p>)} */}
                        </div>
                        <div className='col-md-6'>
                            {pokemon.stats.map((field) => <><strong><p className="card-text" key={Math.random()} >{field.stat.name}: {field.base_stat}</p></strong><Line percent={Number(field.base_stat) / 150 * 100} strokeWidth='1' strokeColor="rgba(0,250,154, 0.6)" /></>)}
                        </div>
                    ...
                    <div className='moveset'>
                        <div className='row'>
                            <h5>Moveset:</h5>
                            |&nbsp;{ pokemon.moves.slice(0, 10).map(({ move }) => <p className='card-text' key={ Math.random() }><strong>{ move.name } |&nbsp;</strong></p>)}
                        </div>
                    </div>
                </div>
            </div><br />
